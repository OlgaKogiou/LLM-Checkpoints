...
class CFManager:
		...
		def __init__(
				...
				try:
					...
					self.rank = 0
				self.telem_path = os.path.join(self.chk_dir, f"cf_telem_rank{self.rank}.csv")
				self._telem = TelemetryLogger(self.telem_path)
		...
		def save(
			...
			persist=False):
				...
				# Prefer threads in distributed context to avoid pickling DDP via spawn
				try:
				    if (os.environ.get('CF_USE_THREAD','0') == '1') or (dist.is_available() and dist.is_initialized()):
				        use_thread = True
				except Exception:
				    pass
				...
				if synchronous:
					...
					# telemetry start
					_size_mb = self.get_chk_size
					# 					self._telem.record('gpu_submit', filepath_sync, self.rank, 0.0, int(_size_mb*1024*1024), 'GPU->CPU submit (sync)')
					_t0 = time.time()
					...
					# 					self._telem.record('cpu_disk_write', filepath_sync, self.rank, elapsed, int(_size_mb*1024*1024), 'Header+lean_state_dict write')
					return
				...
				_size_mb = self.get_chk_size
				# 				self._telem.record('gpu_submit', filepath, self.rank, 0.0, int(_size_mb*1024*1024), 'GPU->CPU submit (async)')
		...
		def initalize_chk_dir(self):
				if os.path.exists(self.chk_dir):
						...
						if os.path.exists(epoch_chk_dir):
								epoch_chk_files = [os.path.splitext(f)[0] for f in os.listdir(epoch_chk_dir) if isfile(os.path.join(epoch_chk_dir, f))]
								epoch_chk_files.sort(key=natural_keys)
								for files in epoch_chk_files:
										self.available_chk_epochs.append(files)
								del epoch_chk_files
						else:
								os.makedirs(epoch_chk_dir, exist_ok=True)
				else:
