...
def main():
    ...
    parser.add_argument("--model_id", type=str, default=default_local_model, help="Model path or Hugging Face repo id")
    parser.add_argument("--resume_from_checkpoint", type=str, default=None, help="Path to checkpoint folder")
    parser.add_argument("--max-global-step", "--max_global_step", dest="max_global_step", type=int, default=-1,
                        help="[DEBUG ONLY] Stop when global optimizer step reaches this value")
    parser.add_argument("--max-step", "--max_step", dest="max_step", type=int, default=-1,
    ...
    parser.add_argument("--save_steps", type=int, default=5, help="Save checkpoint every N global steps")
    ...
    base_output_dir = os.path.join(args.output_root, run_id)
    data_folder_dir = args.data_folder_dir
    trace_logs_dir = f"{base_output_dir}/logs"
    trace_output_dir = f"{trace_logs_dir}/app-{args.local_rank}-of-{torch.cuda.device_count()}"
    ...
    def run():
        ...
        if args.local_rank <= 0:
            ...
            print(f"Model source type: {'local directory' if local_model else 'huggingface hub repo'}")
        ...
        for epoch in ai.pipeline.epoch.iter(range(starting_epoch, num_epochs), include_iter=False):
            if should_stop:
                break
            ...
            while True:
                ...
                with ai.device.transfer:
                    batch = {key: value.to(model_engine.local_rank) for key, value in batch.items()}
                ...
                if model_engine.is_gradient_accumulation_boundary():
                    ...
                    if global_step > 0 and global_step % save_steps == 0:
                        ...
                        with ai.checkpoint.capture:
                            if args.local_rank <= 0:
                                print(f"Saving checkpoint at global step {global_step} with tag '{tag}'...")
